...
static void audio_tx_complete(uint32_t arg, uint8_t *pbuf)
{
...
#if defined(SAVE_AUDIO_DATA) && SAVE_AUDIO_DATA
	memcpy(last_pretx_buf, ptx_buf, AUDIO_DMA_PAGE_SIZE);
#endif
	...
	pretx_queue_ready = xQueueSendFromISR(pretx_record_queue, (void *)last_pretx_buf, &xHigherPriorityTaskWoken_TX);
...
int audio_handle(void *p, void *input, void *output)
{
	...
	for (int i = 0; i < input_item->size; i++) {
		...
		if (cache->idx == AUDIO_DMA_PAGE_SIZE) {
			if (ctx->params.sample_rate == ASR_8KHZ || ctx->params.sample_rate == ASR_16KHZ) {
				// NS needs time needs time, NS is now only process in idex 0 source
				// Todo: NS is nessary in TX path? How to implement in mix mode
				if ((ctx->inited_ns & 0x1) && (ctx->run_ns & 0x1) && (ATAF_NS_CTRL & 0x1) && (cache_idx == 0)) {
