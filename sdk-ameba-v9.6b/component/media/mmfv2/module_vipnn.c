...
static vip_int32_t init_io_buffers(void *p)
{
	...
	for (i = 0; i < ctx->output_count; i++) {
		...
		if (ctx->params.usr_output) {
			vip_create_buffer_from_handle(param, ctx->params.usr_output->pbuf[i], ctx->params.usr_output->size[i], &ctx->output_buffers[i]);
		} else {
...
int vipnn_load_model(nnmodel_t *model, int *size)
{
	...
	*size = nn_f_tell(fd);
	...
	nn_f_read(fd, model->model_content, *size);
	nn_f_close(fd);
