...
int wifi_set_tcp_protocol_keepalive(int socket_fd)
{
	...
	if (keepalive && keepalive_idle && keepalive_interval && keepalive_count) {
		...
		rtw_set_tcp_protocol_keepalive(60 * 1000, 10 * 1000, 5);
	}
...
int wifi_set_tcp_protocol_keepalive_offload(int socket_fd)
{
	...
	wifi_set_tcp_protocol_keepalive(socket_fd);
...
{
	int ret = 0;
	rtw_set_pno_scan(start_window, max_window, increment_steps, scan_period, duration);
	return ret;
}
#endif
